...
namespace Linq_HW {
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            Action<Debtor> printDebtor = d => Console.WriteLine(d);

            //2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag

            //var rhyta = debtors.Where(d =>
            //{
            //    string[] temp = d.Email.Split('@');
            //    return temp[1] == "rhyta.com";
            //}).ToList();
            //rhyta.ForEach(printDebtor);
            ...
            //var listOfDebtor = debtors.Where(d => DateTime.Now.Year - d.BirthDay.Year > 26 && DateTime.Now.Year - d.BirthDay.Year < 36).ToList();
            //listOfDebtor.ForEach(d => Console.WriteLine(d));
            ...
            //list.ForEach(printDebtor);




            //5) Butov adi 18 simvoldan cox olan borclulari cixartmaq

            //var list = debtors.FindAll(d => d.FullName.Count() >= 18);
            //list.ForEach(printDebtor);
            ...
            //var list = debtors.Where(d => d.BirthDay.Month > 11 || d.BirthDay.Month < 3).ToList(); //December January February
            //list.ForEach(printDebtor);
            ...
            //int average = debtors.Sum(d => d.Debt) / debtors.Count;
            ////var list = debtors.Where(d => d.Debt > average).OrderBy(d =>
            ////{
            ////    string[] surname = d.FullName.Split(' ');
            ////    return surname[2];
            ////}).ToList(); //order by surname
            //var list = debtors.Where(d => d.Debt > average).OrderByDescending(d => d.Debt).ToList(); // order by descending debt
            //list.ForEach(printDebtor);
            ...
            //{
            //    string[] surname = d.FullName.Split(' ');
            //    return surname[2] + " " + (DateTime.Now.Year - d.BirthDay.Year) + " " + d.Debt;
            //}).ToList();
            ...
            //var list = debtors.Where(d =>
            //{
            //    var temp = d.FullName.ToCharArray().GroupBy(c => c).OrderByDescending(grp => grp.Count()).First();
            //    return temp.Count() >= 3;
            ...
            //var commonBirthYear = debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(grp => grp.Count()).Select(grp => grp.Key).First();
            //Console.WriteLine(commonBirthYear);
            ...
            //int sumOfDebt = debtors.Sum(d => d.Debt);
            //Console.WriteLine(sumOfDebt);
            ...
            //{
            //    var num = d.Phone.ToCharArray().Where(c => c != '-').ToArray();
            //    var grp = num.GroupBy(a => a);
            ...
            //    {
            //        time = (12 - DateTime.Now.Month) + (d.BirthDay.Month);
            //    }
            //    else
            //        time = d.BirthDay.Month - DateTime.Now.Month;
            //    return d.Debt - time * 500 <= 0;
            //});
            //list.ForEach(printDebtor);
            ...
            //var list = debtors.Where(d =>
            //{
            //    var temp = d.FullName.ToLower();
            //    return temp.Contains('s') && temp.Contains('m') && temp.Contains('i') && temp.Contains('l') && temp.Contains('e');
            //}).ToList();
            //list.ForEach(printDebtor);
        }
    }
